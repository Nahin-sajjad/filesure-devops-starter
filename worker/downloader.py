...
class DocumentDownloader:
    ...
    def find_and_lock_pending_job(self):
        """Find and lock a pending job from MongoDB"""
        try:
            # Clean up expired locks first
            self._cleanup_expired_locks()
            
            # Try to find and lock a pending job
            ...
            if job:
                ...
                logger.info(f"Found and locked pending job {job['_id']} for {job.get('companyName', 'Unknown')}")
                return job
            else:
...
========================================
    ...
    def run_with_job_id(self, job_id):
    ...
    def run_auto_mode(self):
        """Run in auto mode - find and process pending jobs"""
        logger.info("Starting Document Downloader in auto mode...")
        
        try:
            # Find and lock a pending job
            job = self.find_and_lock_pending_job()
            ...
            logger.error(f"Unexpected error in auto mode: {e}")
            sys.exit(1)
...
def health():
    try:
        ...
        downloader.db.command('ping')
        # Check Azure connection if configured
        if downloader.blob_service_client:
            downloader.blob_service_client.get_account_information()
        return "OK", 200
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if job_id:
        ...
        downloader.run_with_job_id(job_id)
    else:
        # Run in auto mode - find and process pending jobs
        downloader.run_auto_mode()